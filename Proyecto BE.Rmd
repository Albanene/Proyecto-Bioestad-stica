...
## Boxplots
```{r}
boxplots_chicken <- ggplot(base3, aes(x= feed, y= weight, fill= feed)) +
  geom_boxplot() +
  labs(title = "Distribución de los pesos en los distintos grupos", x = "Tipo de alimentación", y = "Peso") + theme_minimal() + theme(plot.title = element_text(size = 16, color = "#64BBCC", face = "italic", "bold", family ="serif"), text = element_text(size = 9), legend.title = element_blank(), legend.text = element_blank(), legend.position='none')
boxplots_chicken
``` 
